...
- the official pool does not exist yet (`OfficialPoolExists`): the strategy creates that key for any registered distribution of KAY9, so an auction launched into an existing pool could never migrate (`docs/SECURITY.md`)
...
7. **How Genesis tells a failed migration from a good one, and what that costs.** A migration that reverts makes the strategy hand the whole raise back to Genesis; one that succeeds spends it on the position and returns dust. So Genesis reads its own ETH balance against half of `currencyRaised`. A balance is something anybody can add to, and this one is read knowing that: whoever gives Genesis at least half the raise after a good migration, and before `settle` has written the outcome down, makes that migration read as failed. `settle` then refuses, `recover` runs, and the launch ends with two pools - the official one holding the real raise, and a hookless one holding the giver's ETH and the leftover supply, both locked - with `poolKey()` naming the second from then on. Nothing is stranded and nothing returns to the giver, so the confusion costs them half the raise, paid into KAY9 liquidity for good (`test_aGiftOfHalfTheRaiseBuysASecondPoolAndStrandsNothing`). It is accepted for what the alternative costs: every other signal available on chain - that the official pool exists, that the lock holds a position on it - can be forged in the *other* direction by a stranger's own distribution on the freed pool key, and reading a failed migration as a good one leaves the whole raise in Genesis with no code path able to spend it. Calling `settle` in the transaction that migrates closes the window entirely, and once the outcome is recorded no gift can reopen it.
8. **Recovery cannot be blocked for good.** `recover` refuses any price but the auction's, which is what stops a stranger choosing the price the raise is minted at. A squatter who initializes the hookless pool at another price delays it by one swap: an empty pool moves to any price limit for nothing, a funded one for a trade against the squatter's own liquidity, and repair and recovery fit in one transaction so nobody can move the price again in between (`test_recoveryOutlastsAnEmptySquattedPool`, `test_recoveryOutlastsAFundedSquattedPool`, `test_repairAndRecoveryInOneCallCannotBeSplit`).
9. **The team is paid after the public, never before.** The vesting dates are fixed at deployment and the launch is not, so `KAY9TeamVesting.release` reverts until `KAY9Genesis.settled()` is true. After that the schedule is the calendar to the second. The gate can delay a tranche and can never bring one forward.
