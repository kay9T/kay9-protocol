...
`EVM_RECENT_WINDOW_NOT_COVERED` with the covered height as evidence, and sit at the floor. The deep and forensic tiers fetch their own windows, and since engine 1.7.0 apply the same rule: a recent window with any block unread — a refused request or an exhausted log budget — raises `EVM_RECENT_WINDOW_PARTIAL` instead of `EVM_NO_RECENT_ACTIVITY` or `EVM_TRADING_PATTERN_ORDINARY`. A wash pattern that was observed is still reported, because a partial sample can show something and cannot show nothing. The
...
**What an empty search proves (engine 1.7.0).** The `Initialize` enumeration reads a bounded window, recorded in `observations.liquidity.enumerated`, because a wider log query is refused. An empty window is only an absence of pools if it reaches back to the token's first block — its deployment at the deep and forensic tiers, its first `Transfer` at the basic tier. Otherwise the result is `EVM_LIQUIDITY_WINDOW_PARTIAL`, unmeasured. `EVM_NO_LIQUIDITY_FOUND` is raised only when the window covers the token's life — whether the search found no pool at all or found only pools reading zero active liquidity, and its title says which. One recent empty pool in a window that does not reach the token's first block is still `EVM_LIQUIDITY_WINDOW_PARTIAL`: an older pool outside the window may hold everything. The window is bounded against the pinned block, not the live head, so two auditors pinned to one block search the same range.
...
- **Liquidity coverage is broad on v4 and partial elsewhere.** A token's Uniswap v4 pools are
  ...
  missed is not evidence about the asset. See `docs/SCORE_CALIBRATION.md` §7.1 for the scan that
  established this.
...
**Tokenised equities as quote assets (engine 1.13.0).** A fifth of Pons graduations pair with a tokenised equity rather than ETH or USDG (46 of 219 read on 2026-09-27, across 18 equities). `dex.quotePricing` now lists META, NVDA, SGOV, DJT, SPCX, IBM, GME and AAPL, which cover 33 of the 46, each with its deepest hookless ETH/asset pools of that date: at most three, only pools holding at least 1 % of the deepest one's liquidity, and at least two, so a dust pool cannot set the median. GLD and MSFT have one deep pool each and stay unpriced.
...
**A holder's funder is where its tokens came from (engine 1.12.0).** `EVM_LINKED_WALLETS` groups the largest holders by who first funded them. That used to be the `from` of the first transfer a holder received, which for a routed purchase is the router: every buyer who used the same router formed one "cluster", and the calibration of 2026-09-26 found the finding, at severity high, on 6 of 10 surviving graduations with a router as the shared funder. The funder is now the address that lost the most tokens in the transaction that first delivered to the holder, so router hops cancel out. A holder whose funder is a market (a pool, a launchpad curve, the pool manager, a pool's hook) counts as resolved and linked to nobody; one funded by issuance or other machinery stays unresolved.

**No unlocked verdict over a pool that weighs nothing (engine 1.12.0).** The v4 lock state is weighted by priced depth, and a pool paired with an asset the engine cannot price weighs nothing however much it holds. When such a pool holds liquidity, `EVM_V4_LP_UNLOCKED` is withheld (`EVM_LP_LOCK_UNKNOWN`, naming the holders read): on 2026-09-26 a locked graduation pool paired with an unpriced tokenised equity was left out and a dust side pool held by an ordinary account read as 100 % of the priced depth. A locked verdict is a lower bound and still stands.

**A token on its curve is not untradeable (engine 1.12.0).** When no pool exists and a launchpad curve still holds some of the token at the analysis block, the liquidity category reports `EVM_LIQUIDITY_BONDING_CURVE`, unmeasured, instead of `EVM_NO_LIQUIDITY_FOUND`: until graduation the curve is the market, and its depth is not read. Both tiers score liquidity after the holder fold for this reason.

**Launchpad curves and lockers are not holders (engine 1.11.0).** A launchpad listed in `dex.bondingCurveLaunchpads` mints a token's supply into a per-token curve and names the curve in its launch event. The holder fold notes the token's first mint, reads that launchpad's launch event in the mint block, and treats the curve it names as a market: excluded from the circulating base, and a source of purchases for the early-buyer heuristics. Only the launchpad's own event, about this token, in that block can make an address a curve. A listed `dex.positionLockers` contract that received the token is excluded from the base too, while its code hash still matches: PonsV2LaunchLocker takes 8.16 % of every graduated Pons token through `lockTokenSupply` and nothing can move it out. If either read is refused, concentration is withheld (`EVM_HOLDERS_UNAVAILABLE`, reason `holder-classification-refused`) rather than reported with a market or a lock counted as a whale.

**The pool search reaches the token's first block (engine 1.12.0).** The core scan reads `Initialize` events in the last 3,000,000 blocks, about three and a half days, because it runs before the holder fold has found the token's first block. Once the fold has, a token older than that window is searched again from its first block to the head, in consecutive 3,000,000-block windows, at most ten (about 35 days). The LP ownership read for a pool then starts at that pool's own `Initialize` block, in windows of the same size. On a graduated launchpad token the oldest pool is the graduation pool, and the last window alone held only later side pools: the calibration of 2026-09-26 found locked graduation tokens reported as thin and unlocked that way. Where the search still cannot reach the first block (older than ten windows, or the wider search refused), depth above the low threshold is still reported, because an older pool can only add to it; depth below it is `EVM_LIQUIDITY_WINDOW_PARTIAL`, and the v4 lock state is `EVM_LP_LOCK_UNKNOWN`. The deep tier searches from the creation block when it found one.

**Who a transaction delivered tokens to (engine 1.11.0).** The early-buyer heuristics group transfers by transaction and credit the addresses whose balance the transaction raised, so the hops of a routed purchase cancel out; a transaction that took tokens out of a pool, a pool's hook or a launchpad curve delivered purchases. `EVM_BUNDLED_BUYS` counts transactions with three or more such net receivers, and also, since 1.12, five or more single-wallet purchases in one block, by at least three wallets, each receiving between 0.1 % and 10 % fewer tokens than the one before (1.11 asked for three, which a preset buy button pressed by a few people in one busy block can produce): the same spend repeated up a rising price, one script buying with many wallets across separate transactions (`transactions` > 1 on the bundle). A v4 pool's hook (`hooks` on the pool observation) is treated as part of its pool, because on Robinhood the Pons hook takes its fee in the token inside every swap. `EVM_EARLY_CONCENTRATION` is not evaluated when less than one per cent of supply was bought in the early window (`EVM_EARLY_VOLUME_NEGLIGIBLE`, bot category), the rule `EVM_NO_MEANINGFUL_FLOAT` already applies to holders. All of these came out of the gate-4 calibration (`docs/SCORE_CALIBRATION.md` §2).
